...
function App() {
  ...
  const toggleSpecialDate = useCallback((dateKey: string, isSpecial: boolean) => {
    setSpecialDates(prev => {
      ...
      // If removing special date status, also clean up invalid shifts
      if (!isSpecial) {
        ...
        if (dayOfWeek !== 0 && currentShifts.includes('9-4')) {
          setSchedule(prevSchedule => ({
            ...prevSchedule,
            [dateKey]: currentShifts.filter(shiftId => shiftId !== '9-4')
          }));
        }
      }
      
      // FIXED: Force refresh calculations when special dates change
      setRefreshKey(prev => prev + 1);
      
      return newSpecialDates;
    });
  }, [setSpecialDates, schedule, setSchedule]);

  // Reset month function
  const handleResetMonth = useCallback(async (year: number, month: number, specificDay?: number) => {
    try {
      if (specificDay) {
